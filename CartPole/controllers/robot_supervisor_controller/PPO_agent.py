...
class PPOAgent:
    ...
    def work(self, agent_input, type_="simple"):
        ...
        """
        agent_input = from_numpy(np.array(agent_input)).float().unsqueeze(0)  # Add batch dimension with unsqueeze  # from "torch" import "from_numpy",
        if self.use_cuda:
            agent_input = agent_input.cuda()
        with no_grad():                                                         # from "torch" import "no_grad"
            action_prob = self.actor_net(agent_input)                           #self.actor_net = Actor(number_of_inputs, number_of_actor_outputs)
