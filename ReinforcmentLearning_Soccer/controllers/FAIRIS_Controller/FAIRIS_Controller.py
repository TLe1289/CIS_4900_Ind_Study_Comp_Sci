...
class FAIRIS (RobotSupervisorEnv):
    ...
    def get_reward(self, action=None):
        ...
        return 0
    def is_done(self):
        if self.get_clusters(self.detections) > self.max_cluster:
            return True
        if self.getTime() > 60:
            return True
        return False
    ...
    def eliminate_repeating_array(self,arr):
        unique_elements = set()
        result = []
        for element in arr:
    ...
    def Scan_Progress(self):
        ...
        array = self.eliminate_repeating_array(detections)
        return array
    ...
    def get_clusters(self,detections):
        ...
        #print(detections)
        for section in range(0,len(detections)-1):
            if (detections[section+1]-detections[section]>10):
                #print("Occurs when first:{}, second:{}".format(detections[section],detections[section+1]))
                cluster = cluster + 1
        return cluster
